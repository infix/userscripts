...
const setLastUpdated = () => {
  lastUpdated.innerText = `Last updated: ${dateDiffInMinutes(new Date(), lastUpdate)} mins ago`
}
...
const setAllImages = async () => {
  ...
  const getMangaId = e => e.firstElementChild.querySelector("a")
    .href.replace(MANGADEX_TITLE_URL, "")
    .split("/")[0];

  /** @type {{mangaId: string, element: HTMLElement}[]} */
  const chapterElements = [...document.querySelectorAll(".chapter-container > .row.no-gutters:not(:first-child)")]
