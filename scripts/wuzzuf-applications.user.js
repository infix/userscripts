...
const getData = () => {
  const newData = [...document.querySelectorAll("div.submitted-apps-container.clearfix > div > div")]
    .map(application => {
      ...
      const viewed = application.querySelector("ul li.state:nth-child(2)").classList.contains("active")
      const state = application.querySelector("ul li.state:nth-child(3) span.status-name").innerText
      return [id, { position, company, state: viewed ? state : "Not seen" }]
    });

  const prevStats = JSON.parse(localStorage.getItem("application-stats") || "{}");
...
const createStatsSpan = (name, count, total, color) => h("span", { style: { color } },
...
const numberSubmitted = +document.querySelector("span.number-of-submitted-apps").innerText;

const btn = h("button", { className: "btn btn-primary btn-primary-inverse", onclick: getData }, "Recompute")
const container = h("div", { style: { ...styles.container, flexDirection: 'column' } }, [
  statsContainer,
  totalCount < numberSubmitted && btn
])

if (totalCount < numberSubmitted)
  statsContainer.style.paddingBottom = '1rem';
